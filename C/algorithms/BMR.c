...
static int is_minimal_transversal(Graph* fullGraph, iList* candidate);
  ...
  * ----------------------------------------------------------------- */
 static void merge_transversals(iListList* global, iListList* local, Graph* fullGraph, iList* Vpartition) {
    int ajoutes = 0;
    for (NodeList* cur = local->head; cur != NULL; cur = cur->next) {

        iList* candidatComplet = deep_copy(Vpartition);
        append_all(candidatComplet, cur->value);

        int isT = is_transversal(fullGraph, candidatComplet);
        int isM = 1;
        for (Node* n = candidatComplet->head; n != NULL; n = n->next) {
            int removed = n->value;
            iList* minus = deep_copy(candidatComplet);
            remove_value(minus, removed);

            int still_trans = is_transversal(fullGraph, minus);
        ...
        int exists = dl_bmr_contains_list(global, candidatComplet);

        if (isT && isM && !exists) {
            append_list(global, candidatComplet);
            ajoutes++;
        } else {
            free_list(candidatComplet);
        }
    }
}
 ...
 static void build_subgraph_excluding_vertex(Graph* original, Graph* subgraph, int excludedV, iList* Vpartition);
 ...
 void BMR_algorithm(Graph* graph, iList* Vpartition, int depth, Graph* fullGraph)
 {
     ...
     while (cur) {
         ...
         build_subgraph_excluding_vertex(graph, Epartition, vi, Vpartition); 
         ...
         if (Epartition->nb_edges < 1 || Epartition->nb_edges == graph->nb_edges) {
        } else if (Epartition->nb_edges >= 8 && a >= 5000 ) {
            BMR_algorithm(Epartition, Vpartition, depth+1, fullGraph);
        } else {
            iListList* subTransversals = DL_BMR(Epartition);

            merge_transversals(globalTransversals, subTransversals, fullGraph, Vpartition);
 ...
 * ne contiennent pas 'excludedV' ET en retirant aussi les sommets
 * de Vpartition de ces arêtes (logique BMR).
 * ----------------------------------------------------------------- */
static void build_subgraph_excluding_vertex(Graph* original, Graph* subgraph, int excludedV, iList* Vpartition)
{
    ...
    for (int i = 0; i < original->nb_edges; i++) {
        ...
        if (!contains(e->vertices, excludedV)) {
            iList* newVertices = create_list();

            for (Node* v = e->vertices->head; v != NULL; v = v->next) {
                if (!contains(Vpartition, v->value)) {
                    append(newVertices, v->value);
                }
            }

            if (newVertices->size > 0) {
                add_edge_minimal(subgraph, newVertices);
            } else {
