...
// Cette variante est une idée qui malheureusement donne des résultats faux
void DL_singl(Graph* graph) {
    //printf("\nDL algorithm\n");
    ...
    for (int i = 1; i < graph->nb_edges; i++) {
        ...
        if (Trguaranteed->head == NULL || graph->edges[i]->vertices->size != 1) {
